...
class DomainFronter:
    ...
    def __init__(self, config: dict):
        ...
        self._pool_max = self._cfg_int(config, "pool_max", POOL_MAX, minimum=1)
        self._conn_ttl = self._cfg_float(config, "conn_ttl", CONN_TTL, minimum=5.0)
        self._semaphore = asyncio.Semaphore(
            self._cfg_int(config, "relay_concurrency", SEMAPHORE_MAX, minimum=1)
        )
        self._warmed = False
        self._refilling = False
        self._pool_min_idle = self._cfg_int(
            config, "pool_min_idle", POOL_MIN_IDLE, minimum=0,
        )
        ...
        self._batch_window_micro = self._cfg_float(
            config, "batch_window_micro", BATCH_WINDOW_MICRO, minimum=0.001,
        )
        self._batch_window_macro = self._cfg_float(
            config, "batch_window_macro", BATCH_WINDOW_MACRO, minimum=0.001,
        )
        ...
        log.info(
            ...
            self._pool_max,
            self._pool_min_idle,
            "on" if self._batch_enabled else "off",
            self._batch_max,
            "on" if self._parallel_ranges_enabled else "off",
        )
    ...
    async def _acquire(self):
        ...
        async with self._pool_lock:
            while self._pool:
                ...
                with contextlib.suppress(Exception):
                    writer.close()
        async with asyncio.timeout(self._tls_connect_timeout):
    ...
    @classmethod
    def _log_url(cls, url_or_host: str | None) -> str:
        host = cls._host_key(url_or_host)
        return host or "-"
    ...
    async def _relay_payload_h1(self, payload: dict) -> bytes:
        ...
        async with self._semaphore:
            for attempt in range(attempts):
                try:
                    async with asyncio.timeout(self._relay_timeout):
                        return await self._relay_single(payload)
    ...
    async def _prewarm_one_sid(self, sid: str, payload: bytes,
        ...
        try:
            ...
            async with asyncio.timeout(15):
                status, _, body = await self._h2.request(
        ...
        try:
            ...
            t0 = time.perf_counter()
            async with asyncio.timeout(15):
                status, _, _ = await self._h2.request(
    ...
    async def relay_parallel(self, method: str, url: str,
        ...
        """
        if not self._parallel_ranges_enabled:
    ...
    async def stream_parallel_download(self, url: str, headers: dict,
        ...
        if min_size > 0 and total_size < min_size:
            log.info(
                ...
                total_size, min_size, self._log_url(url),
            )
            return False
        if max_chunks > 0:
            required_chunk_size = max(
        ...
        with tempfile.TemporaryFile(prefix="tg_domain_relay_dl_") as temp_file:
            writer.write(self._render_streaming_headers(resp_hdrs, total_size))
            writer.write(resp_body)
            await writer.drain()
            ...
            try:
                ...
                log.error(
                    "Parallel streaming download failed (%s): %s",
                    self._log_url(url),
                    exc,
                )
                cancel_event.set()
    ...
    async def _batch_send(self, batch: list):
        ...
        if len(batch) == 1:
            ...
            try:
                ...
                # Fallback: send individually using modern TaskGroup
                async with asyncio.TaskGroup() as tg:
                    for payload, future in batch:
                        tg.create_task(self._relay_fallback(payload, future))
    ...
    async def _relay_with_retry(self, payload: dict) -> bytes:
        ...
        if (attempts > 1
            ...
            try:
                async with asyncio.timeout(self._relay_timeout):
                    result = await self._relay_fanout(payload)
                self._record_h2_success()
                return result
            except Exception as e:
                self._record_h2_failure(e)
        ...
        if self._h2_available():
            for attempt in range(attempts):
                ...
                try:
                    async with asyncio.timeout(self._relay_timeout):
                        result = await self._relay_single_h2(payload, sid=sid)
                    self._record_h2_success()
                    return result
        ...
        async with self._semaphore:
            for attempt in range(attempts):
                ...
                try:
                    async with asyncio.timeout(self._relay_timeout):
                        return await self._relay_single(payload, sid=sid)
                except _RelayBadResponse as e:
                    self._blacklist_sid(sid, reason=str(e)[:40])
                    if (attempt < attempts - 1
                            and self._next_alt_sid(tried_sids) is not None):
